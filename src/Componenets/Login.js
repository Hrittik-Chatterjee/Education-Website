...
const Login = () => {
  ...
  const handleSubmit = event => {
    ...
    }).catch(error=> {if(error.message.includes('password')){
      setShowError('Password not matched')
    }else if(error.message.includes('user-not-found')){
      setShowError('There is no user registered with this Email address')
    }else{
  ...
  return (
    <div className='flex justify-center items-center pt-8'>
      <div className='flex flex-col max-w-md p-6 rounded-md sm:p-10 bg-rose-100 text-gray-900'>
        <p className="text-red-900">{showError}</p>
        <div className='mb-8 text-center'>
          <h1 className='my-3 text-4xl font-bold'>Log in</h1>
          <p className='text-sm text-blue-400'>
            Log in to get some premium features
          </p>
        </div>
